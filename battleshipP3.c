...
int main(int argc, char **argv)
{
	...
	if(mode < 2){
		...
		//Wait for player
		playerfd = accept(sfd, (struct sockaddr*)&playerSoc, &len);
		
		struct PlayerMove move;
		strcpy(move.moveString, "move");
		move.player.shipsAlive = 4;
		strcpy(move.player.playerName, "paul");
		move.displayName = 1;
		move.hit = 1;
		move.coord.x = 4;
		move.coord.y = -3;
		
		if(sendMove(playerfd, &move))
			printf("success");
		else
			printf("failure");
